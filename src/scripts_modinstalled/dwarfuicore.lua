...
---Runs one named module and adds Core-specific context to reconstruction errors.
---@param script_name string
...
---Retires the current tooltip presenter or its partial render-hook owner.
local function retire_tooltip()
    ...
    local render_hook = find_loaded_script_environment(
        TOOLTIP_RENDER_HOOK_SCRIPT)
    if render_hook and render_hook.manager and
            type(render_hook.manager.shutdown) == 'function' then
        render_hook.manager:shutdown()
    end
end
...
local function retire_context_menu()
    ...
    if registration and registration.manager and
