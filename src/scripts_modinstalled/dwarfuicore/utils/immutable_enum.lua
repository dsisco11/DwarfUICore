...
-- Immutable numeric enum construction for closed DwarfUICore discriminator sets.
...
function define(values, label)
    assert(type(values) == 'table',
        'DwarfUICore immutable enum values must be a table.')
    label = label or 'enum'
    local data = {}
    local seen = {}
    for name, value in pairs(values) do
        assert(type(name) == 'string' and type(value) == 'number' and
                value == value,
            'DwarfUICore enum names must be strings and values must be numbers.')
        assert(not seen[value],
            ('DwarfUICore %s contains duplicate value %s.'):format(
