...
describe('Input Event UI root collection', function()
    it('deduplicates native, current, overlay, and Core roots', function()
        ...
        local collector = load_collector({native={widgets=native}, enabled={one=true},
    ...
    it('fails closed for unavailable native or malformed overlay state', function()
        local collector = load_collector({native=nil, enabled={}, overlay_state={db={}}})
        assert.is_nil(collector.collect({}, {}))
        collector = load_collector({native={widgets={}}, enabled={}, overlay_state=nil})
        assert.is_nil(collector.collect({}, {}))
        collector = load_collector({native={widgets={}}, enabled={bad=false},
            overlay_state={db={bad={widget={}}}}})
        assert.is_nil(collector.collect({}, {}))
    end)
end)
