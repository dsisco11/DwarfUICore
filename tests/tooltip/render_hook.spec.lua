...
describe('DwarfUICore tooltip render-hook manager', function()
    ...
    it('preserves owning overlay wrappers and repairs true replacement',
        ...
        local DwarfUICore_inner = first_overlay.render_viewscreen_widgets
        local foreign_record = {predecessor=DwarfUICore_inner}
        ...
        local function restore_foreign()
            assert.is_equal(foreign,
                first_overlay.render_viewscreen_widgets)
            first_overlay.render_viewscreen_widgets = DwarfUICore_inner
        end
        ...
        local reorderable = function(...)
            ...
            local returns = table.pack(DwarfUICore_inner(...))
    ...
    it('preserves an owning screen wrapper and its exact restoration',
        ...
        local DwarfUICore_inner = rawget(screen, 'onRender')
        local foreign = function(...)
            table.insert(events, 'foreign-before')
            local returns = table.pack(DwarfUICore_inner(...))
            table.insert(events, 'foreign-after')
        ...
        local function restore_foreign()
            ...
            rawset(screen, 'onRender', DwarfUICore_inner)
        end
        assert.has_no.errors(restore_foreign)
    ...
    it('repairs a screen method that no longer chains DwarfUICore', function()
    ...
    it('rejects incompatible process state versions', function()
        ...
        assert.has_error(function()
            load_hook(process,
                function() return overlay_with(function() end) end)
        end, 'Conflicting DwarfUICore tooltip render-hook versions: ' ..
